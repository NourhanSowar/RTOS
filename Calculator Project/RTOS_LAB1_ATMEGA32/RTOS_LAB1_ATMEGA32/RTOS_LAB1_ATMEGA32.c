...
/*define handles */
xTaskHandle   xwelcome ;
xTaskHandle   xblinking ;
xTaskHandle   xoperator ;
/* define Semaphores */

xSemaphoreHandle Blinking;
...
int main( void )
{
	...
	/* creating Tasks */
	xTaskCreate( PeriodicTask, "WelcomeTask", 300, (void*)1 , 3 , &xwelcome );
	xTaskCreate( BlinkingPatternTask, "BlinkTask", 200, NULL,2 , &xblinking);
	xTaskCreate( OperationTask , "Task_Name", 200, NULL, 1, &xoperator);
...
void BlinkingPatternTask(void * pvParameters )
{
	...
	if (xSemaphoreTake(Blinking,portMAX_DELAY) == pdTRUE)
	{
		while(1)
		{
			/* first  rising Stage */
